<x-layouts.admin>
    ...
    <div class="max-w-2xl mx-auto space-y-6">
        {{-- Header --}}
        <div>
            <h1 class="text-xl font-extrabold text-slate-900 tracking-tight font-display">Edit Fasilitas (POI)</h1>
            <p class="mt-1.5 text-xs font-semibold text-slate-500">Perbarui detail fasilitas publik di bawah, atau seret penanda di peta untuk memposisikan ulang titik koordinat.</p>
        </div>

        {{-- Form Card --}}
        <section class="card p-6 bg-white border border-slate-200/50 shadow-sm overflow-hidden mb-12">
            <form method="POST" action="{{ route('admin.amenities.update', $amenity->id) }}" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input id="name" name="name" type="text" value="{{ old('name', $amenity->name) }}" class="input @error('name') ring-2 ring-rose-200 border-rose-300 @enderror" placeholder="cth. RSUD Abdul Wahab Sjahranie" required />
                        @error('name')
                    ...
                    <div>
                        ...
                        <input id="typeInput" name="type" type="text" value="{{ old('type', $amenity->type) }}" class="input @error('type') ring-2 ring-rose-200 border-rose-300 @enderror" placeholder="cth. Sekolah, Rumah Sakit, Mall, Masjid" required />
                ...
                <div class="rounded-2xl bg-slate-50 border border-slate-200/50 p-4 space-y-3">
                    <div>
                        <div class="text-xs font-bold text-slate-900">Koordinat Lokasi</div>
                        <p class="text-[10px] text-slate-400 font-semibold mt-0.5 font-medium">Klik peta atau seret marker untuk mengubah posisi.</p>
                    </div>
                    
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="text-[9px] font-bold text-slate-500 uppercase tracking-wider block mb-1">Latitude</label>
                            <input id="latDisplay" type="text" class="input bg-white font-mono text-xs text-slate-700" readonly />
                        </div>
                        <div>
                            <label class="text-[9px] font-bold text-slate-500 uppercase tracking-wider block mb-1">Longitude</label>
                            <input id="lngDisplay" type="text" class="input bg-white font-mono text-xs text-slate-700" readonly />
                        </div>
                    </div>
                ...
                <div class="flex gap-3 pt-2">
                    ...
                    <button type="submit" class="btn btn-primary flex-1 border-0 font-bold cursor-pointer">Simpan Perubahan</button>
                </div>
            </form>
        </section>
    </div>
    ...
    @push('scripts')
        ...
        <script>
            ...
            const map = L.map('map', { zoomControl: false }).setView([existingLat, existingLng], 15);
            ...
            typeInput.addEventListener('input', () => {
                const p = marker ? marker.getLatLng() : { lat: existingLat, lng: existingLng };
                setPoint(p.lat, p.lng);
            });
