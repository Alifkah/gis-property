    ...
    <div class="space-y-6">
        ...
        <div>
            <h1 class="text-xl font-extrabold text-slate-900 font-display flex items-center gap-2">
                ...
                <span>Tambah Zona Rawan Banjir</span>
            </h1>
            <p class="mt-1.5 text-xs font-semibold text-slate-500">Isi detail kawasan di bawah, lalu gunakan bilah alat (toolbar) menggambar di peta untuk membuat polygon batas wilayah.</p>
        </div>

        {{-- Form Side-by-Side Grid --}}
        <form method="POST" action="{{ route('admin.flood-zones.store') }}" id="floodZoneForm" class="grid gap-6 lg:grid-cols-[340px_minmax(0,1fr)] mb-12">
            @csrf
            ...
            <div class="space-y-5 self-start">
                <div class="card p-5 bg-white border border-slate-200/50 shadow-sm space-y-4">
                    <div>
                        ...
                        <input id="area_name" name="area_name" type="text" value="{{ old('area_name') }}" class="input @error('area_name') ring-2 ring-rose-200 border-rose-300 @enderror" placeholder="cth. Kelurahan Karang Asam Ulu" required />
                        @error('area_name')
                    ...
                    <div>
                        ...
                        <select id="risk_level" name="risk_level" class="select @error('risk_level') ring-2 ring-rose-200 border-rose-300 @enderror" required>
                ...
                <div id="polygonStatus" class="rounded-2xl bg-rose-50/50 p-4 text-xs font-bold text-rose-600 border border-rose-100/50 flex items-center gap-3">
                ...
                <div class="flex gap-3">
                    ...
                    <button type="submit" id="submitBtn" class="btn btn-primary flex-1 border-0 font-bold cursor-pointer disabled:opacity-40 disabled:pointer-events-none" disabled>Simpan Zona</button>
                </div>
            </div>
    ...
    @push('scripts')
        ...
        <script>
            ...
            map.on(L.Draw.Event.CREATED, function (e) {
                ...
                setStatusSuccess('Polygon berhasil digambar! Klik "Simpan Zona" untuk menyelesaikan.');
            });
            ...
            map.on(L.Draw.Event.EDITED, function () {
                ...
                setStatusSuccess('Polygon berhasil diperbarui! Klik "Simpan Zona".');
            });

            map.on(L.Draw.Event.DELETED, function () {
                geojsonInput.value = '';
                setStatusError('Polygon dihapus. Silakan gambar ulang di peta.');
            });
            ...
            if (savedGeojson) {
                try {
                    ...
                    setStatusSuccess('Polygon terdeteksi. Klik "Simpan Zona" untuk memproses.');
                } catch (e) {}
            }
