<x-layouts.app>
    @php
        ...
        $blueprintSvg = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 600 260" fill="none">'
            ...
            . '<text x="50%" y="215" dominant-baseline="middle" text-anchor="middle" fill="#FAF7F2" font-family="Outfit,system-ui,-apple-system,sans-serif" font-weight="900" font-size="13" letter-spacing="4" opacity="0.8">SAMARINDA PROPERTI GIS</text>'
            . '</svg>';
            ...
            <div class="p-6 sm:p-8">
                ...
                @if ($tab === 'register')
                    <form method="POST" action="{{ route('register.store') }}" class="mt-6 grid gap-4">
                        ...
                        <button type="submit" class="btn btn-primary mt-2 w-full py-3 shadow-md hover:shadow-brand-primary/20 transition">Daftar Akun Baru</button>
                    </form>
                @else
                    <form method="POST" action="{{ route('login.store') }}" class="mt-6 grid gap-4">
                        ...
                        <button type="submit" class="btn btn-primary mt-2 w-full py-3 shadow-md hover:shadow-brand-primary/20 transition">Masuk Ke Platform</button>
                    </form>
                @endif
            </div>
        </div>
    ...
    @push('scripts')
        <style>
            ...
            .tab-active { background:#ffffff; color:var(--color-brand-primary); border: 1px solid rgba(226,232,240,0.8); }
        </style>
    @endpush
</x-layouts.app>
