    ...
    <div class="grid gap-6 min-w-0">
            <section class="card p-6">
                <div>
                    ...
                    <p class="mt-1 text-xs text-slate-500 font-medium">Impor beberapa listing properti sekaligus menggunakan file CSV. Gunakan koordinat spasial (latitude & longitude) yang tepat agar lokasi dapat terpetakan dengan akurat.</p>
                </div>
                ...
                @if ($errors->has('csv_errors'))
                    <div class="mt-4 rounded-2xl bg-rose-50 p-4 ring-1 ring-rose-200">
                        <div class="flex items-center gap-2 text-xs font-bold text-rose-700">
                            ...
                            <span>Ditemukan kesalahan pada file CSV Anda. Impor dibatalkan:</span>
                        </div>
                ...
                <div class="mt-6 grid gap-6 md:grid-cols-2">
                    ...
                    <div>
                        <form method="POST" action="{{ route('seller.listings.import.store') }}" enctype="multipart/form-data" class="grid gap-4">
                            @csrf
                            <div>
                                <label class="text-xs font-bold text-slate-700 block mb-2">Pilih File CSV <span class="text-rose-500">*</span></label>
                                <div class="border-2 border-dashed border-slate-200 rounded-2xl p-6 hover:border-brand-primary/40 hover:bg-brand-primary/5 transition relative group cursor-pointer">
                                    <input type="file" name="csv_file" accept=".csv,text/csv,text/plain" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" required />
                                    <div class="text-center">
                                        ...
                                        <p class="mt-2 text-xs font-bold text-slate-700">Klik atau seret file CSV di sini</p>
                                        <p class="mt-1 text-[10px] text-slate-400 font-medium">Maksimal ukuran file 2 MB</p>
                                    </div>
                                </div>
                            </div>
