        ...
        <aside class="flex w-[380px] shrink-0 flex-col border-r border-slate-200 bg-white">
            <div class="border-b border-slate-200 px-6 py-6">
                <div class="flex flex-wrap items-start justify-between gap-4">
                    <div>
                        ...
                        <div class="mt-1 text-sm text-slate-600">Klik pada peta untuk menentukan titik lokasi listing.
                        </div>
                    </div>
            ...
            <div class="flex-1 overflow-y-auto px-6 py-5">
                <div class="rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-200/70">
                    ...
                    <div class="mt-1 text-xs font-semibold text-slate-500">{{ $property->type }} • Rp
                        {{ number_format((float) $property->price, 0, ',', '.') }}</div>
                </div>

                {{-- Estimation Panel --}}
                <div id="estimationCard"
                    class="mt-5 rounded-2xl bg-brand-primary/5 p-4 border border-brand-primary/10 hidden">
                    <div
                        class="text-xs font-bold text-brand-primary uppercase tracking-wider flex items-center gap-1.5">
                        <svg class="size-4 text-brand-primary animate-pulse" fill="none" viewBox="0 0 24 24"
                    ...
                    <div class="mt-2.5">
                        <div class="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Rekomendasi Kisaran:
                        </div>
                    ...
                    <div class="mt-2.5 pt-2.5 border-t border-brand-primary/10">
                        <div class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">Faktor
                            Pendukung Spasial:</div>
                        <ul id="estFactors" class="space-y-1.5 text-[11px] font-semibold text-slate-600"></ul>
                    </div>
                    <div class="text-[10px] text-slate-400 mt-2.5 font-medium leading-relaxed">Berdasarkan data spasial
                        kecamatan <span id="estDistrict" class="font-bold text-slate-700"></span>.</div>
                </div>

                <form method="POST"
                    action="{{ route('seller.listings.location.update', ['property' => $property->id]) }}"
                    ...
                    <div class="grid gap-3 sm:grid-cols-2">
                        <div>
                            ...
                            <input id="latInput" name="lat" type="number" class="input mt-1"
                                value="{{ old('lat', $point['lat']) }}" step="0.000001" required />
                            @error('lat')
                        ...
                        <div>
                            ...
                            <input id="lngInput" name="lng" type="number" class="input mt-1"
                                value="{{ old('lng', $point['lng']) }}" step="0.000001" required />
                            @error('lng')
                                <div class="mt-1 text-xs font-semibold text-rose-600">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <a href="{{ route('seller.listings.edit', ['property' => $property->id]) }}"
                        class="btn btn-outline w-full text-center">Kembali ke Data</a>
                    <a href="{{ route('seller.listings.index') }}" class="btn btn-outline w-full text-center">Kembali ke
                        Dashboard</a>
                </form>
            </div>
        </aside>
        ...
        <div class="relative flex-1">
            ...
            <div class="absolute left-4 top-4 w-[320px] card p-4 z-[9999] shadow-md">
                ...
                <div class="mt-2 text-xs font-semibold text-slate-500 leading-relaxed">
                    Klik di mana saja pada peta untuk menentukan posisi properti, atau seret penanda pin untuk
                    penempatan yang lebih presisi.
                </div>
            </div>
        </div>
    </div>
